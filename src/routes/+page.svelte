<script lang="ts">
  ...
  import NetworkMonitoring from '$lib/components/NetworkMonitoring.svelte'; // Component Handover hôm trước
...
<div class="flex h-screen bg-gray-900 text-white overflow-hidden">
  <aside class="w-64 border-r border-gray-700 bg-gray-800 shrink-0">
    <SidebarWrapper class="bg-gray-800">
      ...
      <SidebarGroup>
        <SidebarItem label="Quỹ đạo & Phủ sóng" on:click={() => activeTab = 'orbit'} class="text-gray-300 hover:text-white cursor-pointer hover:bg-gray-700 {activeTab === 'orbit' ? 'bg-gray-700 text-white' : ''}">
          <svelte:fragment slot="icon">
            <GlobeSolid class="w-5 h-5 transition duration-75 {activeTab === 'orbit' ? 'text-primary-500' : 'text-gray-400'}" />
          </svelte:fragment>
        </SidebarItem>

        <SidebarItem label="Giám sát Core Network" on:click={() => activeTab = 'network'} class="text-gray-300 hover:text-white cursor-pointer hover:bg-gray-700 {activeTab === 'network' ? 'bg-gray-700 text-white' : ''}">
          <svelte:fragment slot="icon">
